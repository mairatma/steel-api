...
Templates.ApiBuilder.param = function(opt_data, opt_ignored, opt_ijData) {
  ...
  var typesMap1__soy109 = {'param': 0, 'path': 1};
  output += soy.$$escapeHtml(Templates.Select.content({buttonClass: 'btn btn-default form-control dropdown-select', events: {selectedIndexChanged: opt_data.id + ':handleInSelectedIndexChanged_'}, id: opt_data.id + '-inSelect' + opt_data.index, label: '', items: ['Param', 'Path'], selectedIndex: opt_data.param['in'] ? typesMap1__soy109[opt_data.param['in']] : 0}, null, opt_ijData)) + '</div><div class="col-md-3"><span class="builder-param-label">Multiple</span>' + soy.$$escapeHtml(Templates.Switcher.content({checked: opt_data.param.multiple ? opt_data.param.multiple : false, events: {checkedChanged: opt_data.id + ':handleMultipleCheckedChanged_'}, elementClasses: 'builder-param-switcher', id: opt_data.id + '-multipleSwitcher' + opt_data.index}, null, opt_ijData)) + '</div></div><div class="row builder-param-item-row"><div class="col-md-8"><input type="text" class="form-control" placeholder="Validator" value="' + soy.$$escapeHtmlAttribute(opt_data.param.validator ? opt_data.param.validator : '') + '" data-name="validator" /></div></div></div></div>';
  return soydata.VERY_UNSAFE.ordainSanitizedHtml(output);
};
if (goog.DEBUG) {
...
Templates.ApiBuilder.auth = function(opt_data, opt_ignored, opt_ijData) {
  ...
  var roleList130 = opt_data.roles;
  var roleListLen130 = roleList130.length;
  for (var roleIndex130 = 0; roleIndex130 < roleListLen130; roleIndex130++) {
    var roleData130 = roleList130[roleIndex130];
    output += '<div>' + soy.$$escapeHtml(Templates.Switcher.content({checked: opt_data.auth.roles[roleData130], events: {checkedChanged: opt_data.id + ':handleRoleCheckedChanged_'}, elementClasses: 'builder-param-switcher', id: opt_data.id + '-rolesSwitcher' + roleData130}, null, opt_ijData)) + '<span class="builder-param-label"> ' + soy.$$escapeHtml(roleData130) + '</span></div>';
  }
  output += '</div><div class="builder-section-auth-permissions col-md-6"><p class="api-section-title">Permissions</p>';
  var permissionList142 = opt_data.permissions;
  ...
  for (var permissionIndex142 = 0; permissionIndex142 < permissionListLen142; permissionIndex142++) {
    ...
    output += '<div>' + soy.$$escapeHtml(Templates.Switcher.content({checked: opt_data.auth.permissions[permissionData142], events: {checkedChanged: opt_data.id + ':handlePermissionCheckedChanged_'}, elementClasses: 'builder-param-switcher', id: opt_data.id + '-permissionsSwitcher' + permissionData142}, null, opt_ijData)) + '<span class="builder-param-label"> ' + soy.$$escapeHtml(permissionData142) + '</span></div>';
  }
