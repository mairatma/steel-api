...
Templates.ApiBuilder.auth = function(opt_data, opt_ignored, opt_ijData) {
  ...
  if (opt_data.roles && opt_data.roles.length) {
    ...
    var roleList164 = opt_data.roles;
    var roleListLen164 = roleList164.length;
    for (var roleIndex164 = 0; roleIndex164 < roleListLen164; roleIndex164++) {
      var roleData164 = roleList164[roleIndex164];
      output += '<div class="builder-param-switcher-container">';
      var roleChecked__soy166 = '' + ('' + Templates.ApiBuilder.arrHasItem({array: opt_data.auth.roles, item: roleData164}, null, opt_ijData));
      roleChecked__soy166 = soydata.$$markUnsanitizedTextForInternalBlocks(roleChecked__soy166);
      output += soy.$$escapeHtml(Templates.Switcher.content({checked: ('' + roleChecked__soy166).indexOf('true') != -1, events: {checkedChanged: opt_data.id + ':handleRoleCheckedChanged_'}, elementClasses: 'builder-param-switcher', id: opt_data.id + '-rolesSwitcher' + roleData164}, null, opt_ijData)) + '<span class="builder-param-label"> ' + soy.$$escapeHtml(roleData164) + '</span></div>';
    }
    output += '</div>';
  }
  if (opt_data.permissions && opt_data.permissions.length) {
    ...
    var permissionList183 = opt_data.permissions;
    var permissionListLen183 = permissionList183.length;
    for (var permissionIndex183 = 0; permissionIndex183 < permissionListLen183; permissionIndex183++) {
      var permissionData183 = permissionList183[permissionIndex183];
      output += '<div class="builder-param-switcher-container">';
      var permissionChecked__soy185 = '' + ('' + Templates.ApiBuilder.arrHasItem({array: opt_data.auth.permissions, item: permissionData183}, null, opt_ijData));
      permissionChecked__soy185 = soydata.$$markUnsanitizedTextForInternalBlocks(permissionChecked__soy185);
      output += soy.$$escapeHtml(Templates.Switcher.content({checked: ('' + permissionChecked__soy185).indexOf('true') != -1, events: {checkedChanged: opt_data.id + ':handlePermissionCheckedChanged_'}, elementClasses: 'builder-param-switcher', id: opt_data.id + '-permissionsSwitcher' + permissionData183}, null, opt_ijData)) + '<span class="builder-param-label"> ' + soy.$$escapeHtml(permissionData183) + '</span></div>';
    }
    output += '</div>';
  }
...
Templates.ApiBuilder.arrHasItem = function(opt_data, opt_ignored, opt_ijData) {
  var output = '';
  if (opt_data.array) {
    var arrItemList205 = opt_data.array;
    var arrItemListLen205 = arrItemList205.length;
    for (var arrItemIndex205 = 0; arrItemIndex205 < arrItemListLen205; arrItemIndex205++) {
      var arrItemData205 = arrItemList205[arrItemIndex205];
      output += (opt_data.item == arrItemData205) ? '\'true\'' : '';
    }
  }
  return soydata.VERY_UNSAFE.ordainSanitizedHtml(output);
};
