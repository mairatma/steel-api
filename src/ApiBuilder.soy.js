...
Templates.ApiBuilder.title = function(opt_data, opt_ignored, opt_ijData) {
  return soydata.VERY_UNSAFE.ordainSanitizedHtml('<div id="' + soy.$$escapeHtmlAttribute(opt_data.id) + '-title" class="builder-title"><input type="text" class="form-control-borderless" name="title" placeholder="add a title..." value="' + soy.$$escapeHtmlAttribute(opt_data.title ? opt_data.title : '') + '" autocomplete="off" data-oninput="handleInputTitle_" /><div class="builder-visibility"><span class="builder-visibility-label">Visibility</span><div class="builder-visibility-switcher">' + soy.$$escapeHtml(Templates.Switcher.content({checked: opt_data.visibility, events: {checkedChanged: opt_data.id + ':handleVisibilitySwitcherCheckedChanged_'}, id: opt_data.id + '-visibilitySwitcher'}, null, opt_ijData)) + '</div></div></div>');
};
if (goog.DEBUG) {
...
Templates.ApiBuilder.params = function(opt_data, opt_ignored, opt_ijData) {
  ...
  if (opt_data.parameters) {
    var paramList65 = opt_data.parameters;
    var paramListLen65 = paramList65.length;
    for (var paramIndex65 = 0; paramIndex65 < paramListLen65; paramIndex65++) {
      var paramData65 = paramList65[paramIndex65];
      output += Templates.ApiBuilder.param({id: opt_data.id, index: paramIndex65, param: paramData65}, null, opt_ijData);
    }
  }
...
Templates.ApiBuilder.auth = function(opt_data, opt_ignored, opt_ijData) {
  ...
  if (opt_data.roles) {
    var roleList120 = opt_data.roles;
    var roleListLen120 = roleList120.length;
    for (var roleIndex120 = 0; roleIndex120 < roleListLen120; roleIndex120++) {
      var roleData120 = roleList120[roleIndex120];
      output += '<div>' + soy.$$escapeHtml(Templates.Switcher.content({checked: opt_data.auth.roles ? opt_data.auth.roles[roleData120] : false, events: {checkedChanged: opt_data.id + ':handleRoleCheckedChanged_'}, elementClasses: 'builder-param-switcher', id: opt_data.id + '-rolesSwitcher' + roleData120}, null, opt_ijData)) + '<span class="builder-param-label"> ' + soy.$$escapeHtml(roleData120) + '</span></div>';
    }
  }
  ...
  if (opt_data.permissions) {
    var permissionList134 = opt_data.permissions;
    var permissionListLen134 = permissionList134.length;
    for (var permissionIndex134 = 0; permissionIndex134 < permissionListLen134; permissionIndex134++) {
      var permissionData134 = permissionList134[permissionIndex134];
      output += '<div>' + soy.$$escapeHtml(Templates.Switcher.content({checked: opt_data.auth.permissions ? opt_data.auth.permissions[permissionData134] : false, events: {checkedChanged: opt_data.id + ':handlePermissionCheckedChanged_'}, elementClasses: 'builder-param-switcher', id: opt_data.id + '-permissionsSwitcher' + permissionData134}, null, opt_ijData)) + '<span class="builder-param-label"> ' + soy.$$escapeHtml(permissionData134) + '</span></div>';
    }
  }
  output += '</div></div><div class="row"><input type="text" class="form-control" placeholder="Validator" value="' + soy.$$escapeHtmlAttribute(opt_data.auth.validator ? opt_data.auth.validator : '') + '" data-oninput="handleAuthValidatorInput_" /></div></div></div>';
  return soydata.VERY_UNSAFE.ordainSanitizedHtml(output);
};
if (goog.DEBUG) {
 ...
 * @suppress {checkTypes}
 */
Templates.ApiBuilder.save = function(opt_data, opt_ignored, opt_ijData) {
  return soydata.VERY_UNSAFE.ordainSanitizedHtml('<div id="' + soy.$$escapeHtmlAttribute(opt_data.id) + '-save" class="form-group"><button class="btn btn-success" type="submit"><span class="icon-16-confirm"></span> Save</button></div>');
};
if (goog.DEBUG) {
  Templates.ApiBuilder.save.soyTemplateName = 'Templates.ApiBuilder.save';
}
...
Templates.ApiBuilder.save.params = ["id"];
