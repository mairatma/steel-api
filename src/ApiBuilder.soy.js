...
Templates.ApiBuilder.auth = function(opt_data, opt_ignored, opt_ijData) {
  ...
  if (opt_data.roles && opt_data.roles.length) {
    ...
    var roleList179 = opt_data.roles;
    var roleListLen179 = roleList179.length;
    for (var roleIndex179 = 0; roleIndex179 < roleListLen179; roleIndex179++) {
      var roleData179 = roleList179[roleIndex179];
      output += '<div class="builder-param-switcher-container">';
      var roleChecked__soy181 = '' + ('' + Templates.ApiBuilder.arrHasItem({array: opt_data.auth.roles, item: roleData179}, null, opt_ijData));
      roleChecked__soy181 = soydata.$$markUnsanitizedTextForInternalBlocks(roleChecked__soy181);
      output += soy.$$escapeHtml(Templates.Switcher.content({checked: ('' + roleChecked__soy181).indexOf('true') != -1, events: {checkedChanged: opt_data.id + ':handleRoleCheckedChanged_'}, elementClasses: 'builder-param-switcher', id: opt_data.id + '-rolesSwitcher' + roleData179}, null, opt_ijData)) + '<span class="builder-param-label"> ' + soy.$$escapeHtml(roleData179) + '</span></div>';
    }
    output += '</div>';
  }
  if (opt_data.permissions && opt_data.permissions.length) {
    ...
    var permissionList198 = opt_data.permissions;
    var permissionListLen198 = permissionList198.length;
    for (var permissionIndex198 = 0; permissionIndex198 < permissionListLen198; permissionIndex198++) {
      var permissionData198 = permissionList198[permissionIndex198];
      output += '<div class="builder-param-switcher-container">';
      var permissionChecked__soy200 = '' + ('' + Templates.ApiBuilder.arrHasItem({array: opt_data.auth.permissions, item: permissionData198}, null, opt_ijData));
      permissionChecked__soy200 = soydata.$$markUnsanitizedTextForInternalBlocks(permissionChecked__soy200);
      output += soy.$$escapeHtml(Templates.Switcher.content({checked: ('' + permissionChecked__soy200).indexOf('true') != -1, events: {checkedChanged: opt_data.id + ':handlePermissionCheckedChanged_'}, elementClasses: 'builder-param-switcher', id: opt_data.id + '-permissionsSwitcher' + permissionData198}, null, opt_ijData)) + '<span class="builder-param-label"> ' + soy.$$escapeHtml(permissionData198) + '</span></div>';
    }
    output += '</div>';
  }
