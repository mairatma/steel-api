...
Templates.ApiBuilder.param = function(opt_data, opt_ignored, opt_ijData) {
  ...
  output += soy.$$escapeHtml(Templates.Select.content({buttonClass: 'btn btn-default form-control dropdown-select', events: {selectedIndexChanged: opt_data.id + ':handleInSelectedIndexChanged_'}, id: opt_data.id + '-inSelect' + opt_data.index, items: ['Param', 'Path'], selectedIndex: opt_data.param['in'] ? typesMap__soy93[opt_data.param['in']] : 0}, null, opt_ijData)) + '</div></div><div class="row builder-param-item-row"><div class="col-md-8"><input type="text" class="form-control" placeholder="Validator" value="' + soy.$$escapeHtmlAttribute(opt_data.param.validator ? opt_data.param.validator : '') + '" data-name="validator" /></div></div></div></div>';
  return soydata.VERY_UNSAFE.ordainSanitizedHtml(output);
};
if (goog.DEBUG) {
