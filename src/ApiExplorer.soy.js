...
Templates.ApiExplorer.title = function(opt_data, opt_ignored, opt_ijData) {
  ...
  if (opt_data.method) {
    var methodNameList244 = opt_data.method;
    var methodNameListLen244 = methodNameList244.length;
    for (var methodNameIndex244 = 0; methodNameIndex244 < methodNameListLen244; methodNameIndex244++) {
      var methodNameData244 = methodNameList244[methodNameIndex244];
      output += '<span class="explorer-title-label label label-sm http-' + soy.$$escapeHtmlAttribute(methodNameData244) + '-bg">' + soy.$$escapeHtml(methodNameData244) + '</span>';
    }
  }
  var visible__soy251 = opt_data.visibility || ! (opt_data.visibility != null);
  output += '<span class="explorer-title-name">' + soy.$$escapeHtml(opt_data.title ? opt_data.title : opt_data.path) + '</span><span class="explorer-visibility show-tooltip" alt="When your API is marked as \'visible\' it means that it can be requested by anyone, whereas \'invisible\' acts like a firewall where only your server can request it"><span class="explorer-visibility-icon ' + soy.$$escapeHtmlAttribute(visible__soy251 ? 'icon-12-check' : 'icon-12-close-short') + '"></span><span class="explorer-visibility-label">' + soy.$$escapeHtml(visible__soy251 ? 'Visible' : 'Invisible') + '</span></span></p>';
...
Templates.ApiExplorer.auth = function(opt_data, opt_ignored, opt_ijData) {
  ...
  if (opt_data.auth && opt_data.auth.roles && opt_data.auth.roles.length) {
    ...
    if (opt_data.auth.roles) {
      var roleList273 = opt_data.auth.roles;
      var roleListLen273 = roleList273.length;
      for (var roleIndex273 = 0; roleIndex273 < roleListLen273; roleIndex273++) {
        var roleData273 = roleList273[roleIndex273];
        output += '<span class="label label-primary"><span class="label-icon icon-12-person"></span> ' + soy.$$escapeHtml(roleData273) + '</span>';
      }
    }
    if (opt_data.auth.permissions) {
      var permissionList280 = opt_data.auth.permissions;
      var permissionListLen280 = permissionList280.length;
      for (var permissionIndex280 = 0; permissionIndex280 < permissionListLen280; permissionIndex280++) {
        var permissionData280 = permissionList280[permissionIndex280];
        output += '<span class="label label-success"><span class="label-icon icon-12-check"></span> ' + soy.$$escapeHtml(permissionData280) + '</span>';
      }
    }
    output += '</div></div>';
  }
...
Templates.ApiExplorer.params = function(opt_data, opt_ignored, opt_ijData) {
  ...
  if (opt_data.parameters && opt_data.parameters.length) {
    ...
    var paramList292 = opt_data.parameters;
    var paramListLen292 = paramList292.length;
    for (var paramIndex292 = 0; paramIndex292 < paramListLen292; paramIndex292++) {
      var paramData292 = paramList292[paramIndex292];
      output += '<tr><td>' + soy.$$escapeHtml(paramData292.name) + '</td><td>' + soy.$$escapeHtml(paramData292.description ? paramData292.description : '') + '</td><td class="text-center">' + soy.$$escapeHtml(paramData292.type ? paramData292.type : '') + '</td><td class="text-center">' + soy.$$escapeHtml(paramData292.value ? paramData292.value : '') + '</td><td class="text-center">' + ((paramData292.required) ? '<span class="explorer-icon-required icon-16-confirm"></span>' : '') + '</td></tr>';
    }
...
Templates.ApiExplorer.tryParams = function(opt_data, opt_ignored, opt_ijData) {
  ...
  if (opt_data.parameters) {
    var paramList314 = opt_data.parameters;
    var paramListLen314 = paramList314.length;
    for (var paramIndex314 = 0; paramIndex314 < paramListLen314; paramIndex314++) {
      var paramData314 = paramList314[paramIndex314];
      output += '<div class="col-md-3"><label for="explorer-param-' + soy.$$escapeHtmlAttribute(paramData314.name) + '" class="api-section-label">' + soy.$$escapeHtml(paramData314.name) + ' ' + ((paramData314.required) ? '<span class="explorer-icon-required">*</span>' : '') + '</label><input id="explorer-param-' + soy.$$escapeHtmlAttribute(paramData314.name) + '" name="' + soy.$$escapeHtmlAttribute(paramData314.name) + '" class="form-control explorer-section-try-param" type="text" autocomplete="off" placeholder="' + soy.$$escapeHtmlAttribute(paramData314.value ? paramData314.value : '') + '" data-index="' + soy.$$escapeHtmlAttribute(paramIndex314) + '" data-oninput="handleParamInput_" ></div>';
    }
  }
...
Templates.ApiExplorer.tryIt = function(opt_data, opt_ignored, opt_ijData) {
  ...
  var methodItems__soy341 = opt_data.method ? opt_data.method : ['get'];
  var selectedIndex__soy342 = opt_data.methodSelectedIndex ? opt_data.methodSelectedIndex : 0;
  output += '<div class="col-md-12"><div class="input-group">' + ((methodItems__soy341.length > 1) ? '<div class="input-group-btn">' + soy.$$escapeHtml(Templates.Select.content({events: {selectedIndexChanged: opt_data.id + ':handleMethodSelectedIndexChanged_'}, id: opt_data.id + '-methodSelect', label: '', items: methodItems__soy341, buttonClass: 'btn btn-default btn-sm dropdown-select-group-left', hiddenFieldName: 'method', selectedIndex: selectedIndex__soy342}, null, opt_ijData)) + '</div>' : '<div class="input-group-addon explorer-section-try-method">' + soy.$$escapeHtml(methodItems__soy341[0]) + '</div>') + '<div class="input-inner-addon input-inner-addon-left"><button data-clipboard data-target="explorer-section-try-input" class="explorer-section-try-icon btn-transparent icon-16-link" type="button"></button><input id="explorer-section-try-input" class="form-control" type="text" value="' + soy.$$escapeHtmlAttribute(opt_data.host ? opt_data.host : '') + soy.$$escapeHtmlAttribute(opt_data.replacedPath ? opt_data.replacedPath : '') + '" readonly></div><div class="input-group-btn"><button class="btn btn-sm btn-accent explorer-section-try-button" type="button" data-onclick="' + soy.$$escapeHtmlAttribute(opt_data.id) + ':handleClickRun_">Run</button></div><div class="input-group-btn">' + Templates.ApiExplorer.tryRealTime({id: opt_data.id, methodName: methodItems__soy341[selectedIndex__soy342], surfaceId: 'tryRealTime'}, null, opt_ijData) + '</div></div></div></div>';
  return soydata.VERY_UNSAFE.ordainSanitizedHtml(output);
};
if (goog.DEBUG) {
