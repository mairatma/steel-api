...
Templates.ApiExplorer.title = function(opt_data, opt_ignored, opt_ijData) {
  ...
  if (opt_data.method) {
    var methodNameList208 = opt_data.method;
    var methodNameListLen208 = methodNameList208.length;
    for (var methodNameIndex208 = 0; methodNameIndex208 < methodNameListLen208; methodNameIndex208++) {
      var methodNameData208 = methodNameList208[methodNameIndex208];
      output += '<span class="explorer-title-label label label-sm http-' + soy.$$escapeHtmlAttribute(methodNameData208) + '-bg">' + soy.$$escapeHtml(methodNameData208) + '</span>';
    }
  }
...
Templates.ApiExplorer.auth = function(opt_data, opt_ignored, opt_ijData) {
  ...
  if (opt_data.auth && opt_data.auth.roles && opt_data.auth.roles.length) {
    ...
    if (opt_data.auth.roles) {
      var roleList236 = opt_data.auth.roles;
      var roleListLen236 = roleList236.length;
      for (var roleIndex236 = 0; roleIndex236 < roleListLen236; roleIndex236++) {
        var roleData236 = roleList236[roleIndex236];
        output += '<span class="label label-primary"><span class="label-icon icon-12-person"></span> ' + soy.$$escapeHtml(roleData236) + '</span>';
      }
    }
    if (opt_data.auth.permissions) {
      var permissionList243 = opt_data.auth.permissions;
      var permissionListLen243 = permissionList243.length;
      for (var permissionIndex243 = 0; permissionIndex243 < permissionListLen243; permissionIndex243++) {
        var permissionData243 = permissionList243[permissionIndex243];
        output += '<span class="label label-success"><span class="label-icon icon-12-check"></span> ' + soy.$$escapeHtml(permissionData243) + '</span>';
      }
    }
    output += '</div></div>';
  }
...
Templates.ApiExplorer.params = function(opt_data, opt_ignored, opt_ijData) {
  ...
  if (opt_data.parameters && opt_data.parameters.length) {
    ...
    for (var paramIndex255 = 0; paramIndex255 < paramListLen255; paramIndex255++) {
      ...
      output += '<tr><td>' + soy.$$escapeHtml(paramData255.name) + '</td><td>' + soy.$$escapeHtml(paramData255.description ? paramData255.description : '') + '</td><td class="text-center">' + soy.$$escapeHtml(paramData255.type ? paramData255.type : '') + '</td><td class="text-center">' + soy.$$escapeHtml(paramData255.value ? paramData255.value : '') + '</td><td class="text-center">' + ((paramData255.required) ? '<span class="explorer-icon-required icon-16-confirm"></span>' : '') + '</td></tr>';
    }
...
Templates.ApiExplorer.tryParams = function(opt_data, opt_ignored, opt_ijData) {
  ...
  if (opt_data.parameters) {
    var paramList277 = opt_data.parameters;
    var paramListLen277 = paramList277.length;
    for (var paramIndex277 = 0; paramIndex277 < paramListLen277; paramIndex277++) {
      var paramData277 = paramList277[paramIndex277];
      output += '<div class="col-md-3"><label for="from">' + soy.$$escapeHtml(paramData277.name) + ' ' + ((paramData277.required) ? '<span class="explorer-icon-required">*</span>' : '') + '</label><input name="' + soy.$$escapeHtmlAttribute(paramData277.name) + '" class="form-control explorer-section-try-param" type="text" autocomplete="off" placeholder="' + soy.$$escapeHtmlAttribute(paramData277.value ? paramData277.value : '') + '" data-index="' + soy.$$escapeHtmlAttribute(paramIndex277) + '" data-oninput="handleParamInput_" ></div>';
    }
  }
